...
<div class="row g-4">
	<div class="col-lg-7">
			...
			<div class="card-body">
				...
				<table class="table table-borderless align-middle mb-0">
					...
					<tr>
						...
						<td>{{ $pengembalian->kondisi_alat ? ucwords(str_replace('_', ' ', $pengembalian->kondisi_alat)) : '-' }}</td>
					</tr>
					<tr>
					...
					<tr>
						<th class="info-label">Metode Pembayaran</th>
						<td>
							<span class="badge bg-info">{{ ucwords(str_replace('_', ' ', $pengembalian->metode_pembayaran)) }}</span>
						</td>
					</tr>
					@endif
					<tr>
		...
		<!-- FORM METODE PEMBAYARAN (hanya jika ada denda) -->
		@if ($dendaValue > 0)
		<div class="card detail-card mt-4">
			<div class="card-body">
				<h5 class="fw-semibold mb-3">Pilih Metode Pembayaran Denda</h5>
				<form action="{{ route('peminjam.pengembalian.update-metode', $pengembalian->id) }}" method="POST">
					@csrf
					@method('PUT')
					<div class="row g-3">
						<div class="col-md-6">
							...
							<select name="metode_pembayaran" id="metode_pembayaran" class="form-select" required>
								...
								<option value="tunai" {{ ($pengembalian->metode_pembayaran ?? '') == 'tunai' ? 'selected' : '' }}>Tunai</option>
								<option value="QRIS" {{ ($pengembalian->metode_pembayaran ?? '') == 'QRIS' ? 'selected' : '' }}>QRIS</option>
							</select>
						</div>
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
		...
		if (metodeSelect) {
			...
			toggleQRIS();
			metodeSelect.addEventListener('change', toggleQRIS);
		}
	});
</script>
@endpush