				...
				<table class="table table-hover align-middle mb-0">
					...
					<tbody data-peminjaman-table>
						@foreach ($peminjamans as $peminjaman)
							@php
								$pinjamDate = $peminjaman->tanggal_pinjam ? \Illuminate\Support\Carbon::parse($peminjaman->tanggal_pinjam) : null;
								$dueDate = $peminjaman->tanggal_kembali ? \Illuminate\Support\Carbon::parse($peminjaman->tanggal_kembali) : null;
							...
							@endphp
							<tr class="peminjaman-row" data-peminjaman-row="{{ $peminjaman->id }}" data-peminjaman-nama="{{ $peminjaman->peminjam->name ?? '-' }}" data-peminjaman-username="{{ $peminjaman->peminjam->username ?? '-' }}" data-alat="{{ $peminjaman->alat->nama_alat ?? '-' }}" data-pinjam-date="{{ $pinjamDate?->format('Y-m-d') }}" data-due-date="{{ $dueDate?->format('Y-m-d') }}" data-status-label="{{ $statusLabel }}" data-status-class="{{ $statusClass }}" data-overdue="{{ $isOverdueNow ? 'true' : 'false' }}">
								<td>
			...
			<div class="selected-summary border rounded-4 p-3 mt-4">
				<div class="d-flex justify-content-between align-items-center mb-3">
					<div>
						...
						<p class="text-muted small mb-0">Data ini otomatis terisi saat Anda memilih baris di tabel.</p>
					</div>
	...
	<select name="peminjaman_id" id="peminjaman_id" class="form-select d-none" aria-hidden="true" tabindex="-1">
		...
		@foreach ($peminjamans as $peminjaman)
			@php
				...
				$isOverdueNow = $dueDate ? $dueDate->isPast() : false;
			@endphp
			<option value="{{ $peminjaman->id }}" data-peminjam="{{ $peminjaman->peminjam->name ?? '-' }}" data-username="{{ $peminjaman->peminjam->username ?? '-' }}" data-alat="{{ $peminjaman->alat->nama_alat ?? '-' }}" data-pinjam-date="{{ $pinjamDate?->format('Y-m-d') }}" data-due-date="{{ $dueDate?->format('Y-m-d') }}" data-status-label="{{ $statusLabel }}" data-status-class="{{ $statusClass }}" data-overdue="{{ $isOverdueNow ? 'true' : 'false' }}" {{ old('peminjaman_id') == $peminjaman->id ? 'selected' : '' }}>
				{{ $peminjaman->peminjam->name ?? 'Peminjam tidak tersedia' }} - {{ $peminjaman->alat->nama_alat ?? 'Alat tidak ditemukan' }}
			</option>
		@endforeach
	</select>
			...
			<div class="card-body">
				...
				<div class="row g-4">
					<div class="col-md-6">
						...
						<input type="date" id="tanggal_pengembalian" name="tanggal_pengembalian" class="form-control" value="{{ old('tanggal_pengembalian') }}" min="{{ $today }}" required>
					</div>
					<div class="col-md-6">
						<label for="kondisi_alat" class="form-label">Kondisi Alat</label>
						<input type="text" id="kondisi_alat" name="kondisi_alat" class="form-control" placeholder="Contoh: Baik, lengkap" value="{{ old('kondisi_alat') }}" required>
					</div>
					<div class="col-12">
						<label for="catatan" class="form-label">Catatan (opsional)</label>
						<textarea name="catatan" id="catatan" rows="4" class="form-control" placeholder="Catatan tambahan mengenai pengembalian">{{ old('catatan') }}</textarea>
					</div>
				</div>
			</div>
		</div>
			...
			<div class="card-body d-flex flex-column gap-4">
				<div>
					...
					<div class="input-group">
						...
						<input type="number" id="denda" name="denda" class="form-control" value="{{ old('denda', 0) }}" min="0" step="1000">
					</div>
				...
				<div>
					<div class="d-flex justify-content-between align-items-center mb-2">
						...
						<div class="btn-group">
							...
							<a href="{{ route('filemanager.list') }}" class="btn btn-sm btn-outline-secondary" target="_blank" rel="noopener">Kelola</a>
						</div>
					</div>
					<select name="file_bukti_pengembalian_id" id="file_bukti_pengembalian_id" class="form-select d-none" aria-hidden="true">
						<option value="" {{ old('file_bukti_pengembalian_id') ? '' : 'selected' }}>Pilih file</option>
						@foreach ($files as $file)
							@php
								$previewPath = asset($file->path ?? $file->file_path);
								$fileName = $file->nama_file ?? $file->file_name ?? 'Tanpa nama';
							@endphp
							<option value="{{ $file->id }}" data-preview="{{ $previewPath }}" data-name="{{ $fileName }}" {{ (string) old('file_bukti_pengembalian_id') === (string) $file->id ? 'selected' : '' }}>
								{{ $fileName }}
							</option>
						@endforeach
					</select>
				...
				<div class="d-grid gap-2 mt-auto">
					<div class="btn-group">
						...
						<a href="{{ route('pengembalian.list') }}" class="btn btn-dark">Kembali</a>
					</div>
				</div>
			</div>
		</div>
