    ...
    <div class="card-body">
        <form method="GET" action="{{ route('admin.log.index') }}">
            <div class="d-flex gap-3 align-items-end flex-wrap">
                ...
                <div class="d-flex flex-column gap-1 flex-grow-1" style="min-width: 160px;">
                    <label class="small fw-semibold text-muted">Cari</label>
                    <input type="text" name="search" class="form-control form-control-sm"
                        placeholder="Nama, deskripsi, aksi..." value="{{ request('search') }}">
                </div>

                <div class="d-flex flex-column gap-1" style="min-width: 130px;">
                    <label class="small fw-semibold text-muted">Aksi</label>
                ...
                <div class="d-flex flex-column gap-1" style="min-width: 140px;">
                    <label class="small fw-semibold text-muted">Dari tanggal</label>
                    <input type="date" name="start_date" class="form-control form-control-sm"
                        value="{{ request('start_date') }}">
                </div>

                <div class="d-flex flex-column gap-1" style="min-width: 140px;">
                    <label class="small fw-semibold text-muted">Sampai tanggal</label>
                    <input type="date" name="end_date" class="form-control form-control-sm"
                        value="{{ request('end_date') }}">
                </div>

                <div class="d-flex gap-2 pb-1">
                    <button type="submit" class="btn btn-success btn-sm px-3">Filter</button>
                    <a href="{{ route('admin.log.index') }}" class="btn btn-outline-secondary btn-sm px-3">Reset</a>
                    <a href="{{ route('admin.log.export-pdf') }}?{{ http_build_query(request()->only(['search', 'action', 'start_date', 'end_date'])) }}"
                        class="btn btn-sm px-3 btn-export" target="_blank">Export PDF</a>
                </div>
